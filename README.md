...
**인증(Authentication)은 사용자가 누구인지 확인하는 프로세스 이다.** 이는 사용자 아이디와 비밀번호 조합으로 자주 수행되지만, 생체 인식 등 다른방법을 포함할 수 있다.

**권한(Authorization)은 사용자가 가지고 있는 엑세스 권한을 확인하는 프로세스이다.** 권한 부여 정책 과 규칙을 사용하여 리소스에 엑세스 하거나 시스템 데이터를 변경할 수 있는 권한이 있는지 확인한다. 권한부여는 일반적으로 사용자 인증이 이루어진 후에 실행된다.

### 마이크로 서비스에서의 인증 및 권한부여

모놀리식 시스템에서는 인증과 권한 부여를 도입하면 전체 시스템이 즉시 이점을 누릴 수 있다. 그러나 마이크로 서비스에서는 각 마이크로 서비스에 인증 및 권한 부여 로직을 구현하지 않는다면 불필요한 중복이 발생된다. 그래서 인증 권한 부여 도메인을 서비스하는 마이크로 서비스를 도입해야한다, 이렇게 운영 시에는 사용자가 인증될 수있다.

![1747815908585](image/README/1747815908585.png)

분리된 서비스 상태를 유지하고 작업을 다른 서비스에 작동할때 이는 API 게이트 웨이를 통해 달성이 가능하다.

### API 게이트웨이 및 인증

지금까지는 E-Commerce 애플리케이션에 요청을 할 때마다 각 마이크로서비스와 직접 통신해야 했습니다. 이 방법은 더 많은 마이크로서비스를 도입하고 모든 URL을 기억해야 하기 때문에 매우 빠르게 통제할 수 없게 될 수 있습니다. 만약 단일 지점이 있어서 매번 기본 URL (port 번호)를 변경할 필요가 없게하기 위해 API 게이트 웨이가 존재한다 아래 예시의 사진과 같다,

![1747816028114](image/README/1747816028114.png)


HTTP 마이크로서비스에 대한 단일 진입점을 제공하는 것 외에도 API 게이트웨이를 사용하여 각 요청의 인증 및 권한 부여와 같은 교차 편집 문제를 처리할 수 있습니다. 이렇게 하면 마이크로서비스가 이 모든 논리를 구현할 필요가 없으며 시스템을 더 깨끗하게 유지할 수 있습니다.

API 게이트웨이는 API의 속도 제한 관리, 각 마이크로서비스에 대한 트래픽 모니터링 등 이 과정에서 다루지 않을 훨씬 더 많은 것을 제공합니다.
